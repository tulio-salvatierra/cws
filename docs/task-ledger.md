...
| CWS-ADMIN-CONSOLIDATE-003 | Codex | Completed with follow-up | Synced lockfile, repaired stale tests, consolidated routes under `/admin`, moved page files into `src/pages/admin`, and applied the private-schema helper migration to staging; all 47 policies now use private helpers, member/non-member RLS checks pass, and targeted advisor warnings are cleared | Decide whether to accept the remaining intentional `create_workspace` and leaked-password warnings, then push/deploy and smoke-test `/admin` routes |
