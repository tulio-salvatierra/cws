...
| CWS-PILOT-READINESS-004 | Codex | In progress — follow-up | Added render smoke coverage and the baseline CWS-001 readiness audit; then added independent content-variant editing and lifecycle status controls with focused interaction coverage, using existing columns and RLS | Implement approval revision resolution, then decision/learning capture; keep export filename/version and outcome fields in a future migration |
| CWS-VERCEL-RELIABILITY-005 | Codex | Completed locally; deployment pending | Added guarded stale-chunk recovery, prevented SPA fallback for missing assets/API/file paths, and bounded/logged the Google Sheets intake request; 62 tests and both Vite/Vercel production builds pass | Commit, push, deploy, then smoke-test a missing asset, campaign navigation, and one intake submission |
