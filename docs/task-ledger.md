...
| CWS-ADMIN-CONSOLIDATE-003 | Codex | Blocked pending security correction | Synced lockfile, repaired stale tests, added and applied migration 013, consolidated routes under `/admin`, and moved page files into `src/pages/admin`; revoking helper execution breaks RLS policy evaluation; MCP recorded the remote migration with a timestamped version | Approve a helper-access correction that preserves RLS while removing direct RPC exposure, align migration history, then rerun advisor and member/non-member checks |
