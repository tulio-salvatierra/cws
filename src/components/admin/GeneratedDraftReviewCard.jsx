...
export default function GeneratedDraftReviewCard({ run, campaigns, onReviewed }) {
  const eligibleCampaigns = useMemo(() => campaigns.filter((campaign) => campaign.channel_id === run.output?.channel_id), [campaigns, run.output?.channel_id])
  ...
  async function review(action) {
    if (!confirmed) return setError('Confirm the review decision before continuing.')
    setSaving(true)
    setError('')
    try {
      ...
      const response = await fetch('/api/review-generated-draft', {
        ...
        body: JSON.stringify({ run_id: run.id, action, feedback, ...(action === 'accept' ? { campaign_id: campaignId, code, working_title: title, draft_text: draftText } : {}) }),
      })
