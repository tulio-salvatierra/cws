...
export default function AgentRunsPage() {
  ...
  const load = useCallback(async () => {
    ...
    if (m.error) return setState({ runs: [], error: m.error.message })
    const r = await supabase.from('agent_runs').select('id, agent_key, command_level, status, input, output, error_message, created_at').eq('workspace_id', m.data.workspace_id).order('created_at', { ascending: false })
    if (r.error) setState({ runs: [], error: r.error.message })
    else setState({ runs: r.data || [], error: '' })
  }, [])
