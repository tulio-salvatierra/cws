  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const [campaigns, channels] = await Promise.all([
        supabase.from('campaigns').select('id, code, title, description, status, updated_at, channel_id').eq('workspace_id', membership.data.workspace_id).order('updated_at', { ascending: false }),
      ...
      if (active) setState({ loading: false, error: '', campaigns: (campaigns.data || []).map((campaign) => ({ ...campaign, channelName: channelNames.get(campaign.channel_id) })) })
    }
