...
const TYPES = ['intro', 'follow_up', 'cold', 'mailing_list']
...
export default async function handler(req, res) {
  ...
  if (type && !TYPES.includes(type)) return res.status(400).json({ ok: false, error: 'type must be intro, follow_up, cold, or mailing_list.' })
